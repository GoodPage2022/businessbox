...
const handler = async (req: NextApiRequest, res: NextApiResponse<any>) => {
    ...
    try {
        ...
        const businesses = await db.collection("collections_Businesses").find({_id: { $in: bodyParams.projects.map((b:any)=>new ObjectId(b)) }}).toArray()
        ...
        const orderTop = await db.collection("collections_toporders").insertOne({
            projects: projects,
            user: (({api_key, i18n, _created, _modified, ...user})=>user)(bodyParams.user),
