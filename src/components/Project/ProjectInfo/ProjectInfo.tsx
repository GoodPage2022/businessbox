  ...
  return (
    <section className='projectInfo'>
      <div className="container projectInfo__container--desc">
        ...
        <p className="projectInfo__text section__primary-text">Клієнт дуже важливий, за клієнтом піде клієнт. Duis feugiat sapien і lectus convallis commodo. Curabitur vulputate lectus не від стрілок, нехай буде багато членів і не приймає. Меценат Tellus Sapien, dapibus a ullamcorper sit amet, hendrerit sit amet ex. Футбол футбольний сезон. But rutrum tellus neque, and eleifend orci convallis molestie. Все життя школи часом буває дитячим салатом чи, піснею. Еней якраз такий м`який, йому треба вкладати в озеро. Перед ним, перш за все, в горлах лікарні стелити ліжка догляду та догляду; Завтра я проведу трохи часу зі стрілками. Але урна самого життя. Хворобливі і вагітні гравці. До того часу лікар хоче знати, що робити.
        </p>

        <ProfileInfo/>
      
        {OurComments.map(({ id, name, mail, date, text, image }) => (
        <Comment key={id} name={name} mail={mail} date={date} image={image} text={text} />
      ))}
      ...
      <h2 className="projectInfo__offers-title title">Схожі пропозиції</h2>
        <ul className="popular__cards">
          {PopularCards.map(({ id, title, description, image }) => (
              <PopularCard key={id} title={title} description={description}   image={image} />
              ))
            }
        </ul>
      </div>
